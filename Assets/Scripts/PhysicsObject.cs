...
public class PhysicsObject : MonoBehaviour
{
    ...
    public bool enabledCollisions = true;
    ...
    public RaycastHit2D CheckVerticalHit(LayerMask layer, float padding = 0.0f, int diry = -2)
    {
        float directionY = (diry==-2?Mathf.Sign(velocity.y):diry);
        float rayLength = skinDist + padding;
        for (int i = 0; i < verticalRayCount; i++)
        {
            Vector2 rayOrigin = (directionY == -1) ? raycastOrigins.bottomLeft : raycastOrigins.topLeft;
    ...
    public void Move(Vector2 velocity)
    {
        ...
        if (velocity.x != 0 && enabledCollisions)
        {
            HorizontalCollisions(ref velocity);
        }
        if (velocity.y != 0 && enabledCollisions)
        {
